	...
	public class ClassController : Controller
	{
		...
		public async Task<IActionResult> Index()
		{
			try
			{
				...
				var classes = JsonSerializer.Deserialize<List<ClassListDto>>(content, JsonOptions.Default) ?? new List<ClassListDto>();
				
				var viewModel = classes
					.Where(c => c != null)
					.Select(c => new ClassListViewModel
					{
						...
						IsActive = c.IsActive,
						OrderBy = c.OrderBy,
						SchoolName = c.SchoolName ?? string.Empty,
		...
		public async Task<IActionResult> Details(Guid id)
		{
			try
			{
				...
				if (classDto == null)
				{
					_logger.LogWarning("Deserialized class DTO is null for ID: {ClassId}", id);
					TempData["ErrorMessage"] = "Class data could not be loaded.";
				...
				{
					Id = classDto.Id,
					Name = classDto.Name ?? string.Empty,
					...
					Status = classDto.Status ?? string.Empty,
					StatusMessage = classDto.StatusMessage ?? string.Empty
				};
		...
		public async Task<IActionResult> Edit(Guid id)
		{
			try
			{
				...
				if (classDto == null)
				{
					_logger.LogWarning("Deserialized class DTO is null for edit, ID: {ClassId}", id);
					TempData["ErrorMessage"] = "Class data could not be loaded for editing.";
					return RedirectToAction(nameof(Index));
				}
